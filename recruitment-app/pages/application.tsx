...
export default function Application() {
  ...
  const [filteredUsers, setFilteredUsers] = useState(users.filter((words, i) => (i<(index+4) && i >= index)));
  const applications_per_page = 4;

  function prevPage(){
    if(index-applications_per_page <= 0){
      setIndex(0);
    } else {
      setIndex(index-4);
    }
    setFilteredUsers(users.filter((words, i) => (i<(index+applications_per_page) && i >= index)));
  }
  function nextPage(){
    if(index+applications_per_page > users.length){
      setIndex(0);
    } else {
      setIndex(index+applications_per_page);
    }
    setFilteredUsers(users.filter((words, i) => (i<(index+applications_per_page) && i >= index)));
  }
